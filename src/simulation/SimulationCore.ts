...
export abstract class SimulationCore {
  ...
  constructor(teamA: TeamConfig, teamB: TeamConfig, initialVelocities: InitialVelocities) {
    ...
    this.bodyA = Bodies.circle(BALL_A_START.x, BALL_A_START.y, teamA.ball.radius, { ...bodyOptionsFromBall(teamA.ball), label: 'ballA' });
    this.bodyB = Bodies.circle(BALL_B_START.x, BALL_B_START.y, teamB.ball.radius, { ...bodyOptionsFromBall(teamB.ball), label: 'ballB' });
  ...
  protected tick(delta: number): void {
    ...
    const speedMults = teams.map(t => this.statusMgr.getSpeedMultiplier(t.id)) as [number, number];
    ...
    this.weapons.updateBullets(scaledDelta, this.hp, this.teamA, this.teamB, this.bodyA, this.bodyB, this.boundApplyHit);
  ...
  protected registerCollisionHandler(): () => void {
    const handler = (e: Matter.IEventCollision<Matter.Engine>) => this.onCollision(e);
